...
/**
 * This is the model class for table "efficiency_report_day".
 *
 * @property integer $id
 ...
 */
class EfficiencyReportDay extends EfficiencyBase
{
    /**
     * @inheritdoc
     */
    ...
    {
        return [
            [['date', 'my_ip', 'proc', 'start_count', 'start_response_time', 'update_count', 'update_response_time', 'stop_count', 'stop_response_time', 'auth_count', 'auth_response_time', 'coa_count', 'coa_response_time', 'dm_count', 'dm_response_time'], 'required'],
            [['date', 'start_count', 'update_count', 'stop_count', 'auth_count', 'coa_count', 'dm_count'], 'integer'],
    ...
    public function getMultiData($times, $data)
    {
        ...
        foreach ($data as $v) {
            ...
            $yAxis[$v['date']][$v['my_ip']]['start_response_time'] = $start = sprintf('%.2f', $v['start_response_time']);
            $yAxis[$v['date']][$v['my_ip']]['update_response_time'] = $update = sprintf('%.2f', $v['update_response_time']);
            $yAxis[$v['date']][$v['my_ip']]['auth_response_time'] = $auth = sprintf('%.2f', $v['auth_response_time']);
            $yAxis[$v['date']][$v['my_ip']]['stop_response_time'] = $stop = sprintf('%.2f', $v['stop_response_time']);
            $yAxis[$v['date']][$v['my_ip']]['coa_response_time'] = $coa = $v['coa_response_time'];
            $yAxis[$v['date']][$v['my_ip']]['dm_response_time'] = $dm = $v['dm_response_time'];
            $table['data'][$date]['data'] = [$date, $start . 'ms', $update.'ms',$stop . 'ms', $auth . 'ms',  $coa . 'ms', $dm . 'ms'];
            $table['data'][$date]['detail'][] = [$v['my_ip'], $date, $start . 'ms', $update.'ms',$stop . 'ms', $auth . 'ms',  $coa . 'ms', $dm . 'ms'];
        }
    ...
    public function getData()
    {
        try {
            foreach ($this->process as $v) {
                $this->process_default = $v;
                $data = $this->getEfficiencyStatus(1);
                $ids[] = $v;
                $rs['data'][$v] = $data;
            }
            $rs['code'] = 200;
