...
/**
 * This is the model class for table "cloud_partitions_hour".
 *
 * @property string $id
 ...
 */
class CloudPartitionsHour extends \yii\db\ActiveRecord
{
    /**
     * @inheritdoc
     */
    ...
    {
        return [
            [['date', 'hour', 'total_bytes', 'free_bytes'], 'integer'],
...
//搜索字段
    ...
    {
        $newParams = [];
        $newParams[':sta'] = (!empty($params) && !empty($params['start_time'])) ? strtotime($params['start_time']) : strtotime(date('Y-m-d'));
        ...
        try {
            ...
            if (!empty($products)) {
                ...
                $query->groupBy(['product_name', 'device_ip', 'mount_point', 'date', 'hour']);
