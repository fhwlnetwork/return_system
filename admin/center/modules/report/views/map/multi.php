    ...
    option = {
        baseOption: {
            timeline: {
                ...
                bottom: 50,
                data: <?=isset($data['times'])?$data['times']:json_encode([])?>,
            },
            title: {
                subtext: '<?= $name?>'
            },
            tooltip: {
                ...
                formatter: function (params) {
                    ...
                    for (var param in params) {
                        ...
                        if (preg.test(sql_type)) {
                            value = getBytesFormat(value);
                        } else if (sql_type == 'time_long') {
            ...
            yAxis: [
                {
                    ...
                    axisLabel: {
                        formatter: function (v) {
                            value = v;
                            if (preg.test(sql_type)) {
                                value = number_format(v / (1024 * 1024), 2) + 'Mb';
                            } else if (sql_type == 'time_long') {
                                value = number_format(v / (60 * 60), 2) + 'Hour';
                            }
