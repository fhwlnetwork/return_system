...
use center\modules\user\models\UserCloundComplaints;
...
class ComplaintsController extends ValidateController
{
    const  COMPLAINTS_PUSH_CLOUD_API = '/complaints/push'; //推送到云端的接口
    const  COMPLAINTS_LIST_CLOUD_API = '/complaints/list'; //获取云端投诉数据接口
    const  COMPLAINTS_DETAIL_CLOUD_API = '/complaints/detail';
    ...
    public function actionIndex()
    {
        ...
        $model = new UserCloundComplaints();
        $attributes = $model->getAttributesList();
        $bugTypes = isset($attributes['bugTypes']) ? $attributes['bugTypes'] : [];
        $bugStates = isset($attributes['bugStates']) ? $attributes['bugStates'] : [];
        $list = $data = [];
        ...
        if (!empty($rs) && $rs['code'] == 0) {
            ...
            $keys = '?' . $keys;
            $url = $rs['prefix'] . self::COMPLAINTS_LIST_CLOUD_API . $keys;
        ...
        if (!empty($data) && $data['code'] == 0) {
            ...
            if (!empty($list)) {
                ...
                foreach ($list as &$v) {
                    unset($v['bug_attach']);
                    unset($v['bug_solution']);
                    $v['bug_type'] = isset($bugTypes[$v['bug_type']]) ? $bugTypes[$v['bug_type']] : '';
                    $v['bug_state'] = isset($bugStates[$v['bug_state']]) ? $bugStates[$v['bug_state']] : '';
                }
            }
            $pagination = new Pagination([
                'defaultPageSize' => $offset,
    ...
    public function actionView($id)
    {
        ...
        $data = [];
        if (!empty($rs) && $rs['code'] == 0) {
            $url = $rs['prefix'] . self::COMPLAINTS_DETAIL_CLOUD_API;
            $params['access_token'] = $rs['data']['access_token'];
            $params['uid'] = $rs['data']['uid'];
            ...
            if (!empty($res) && $res['code'] == 0) {
                ...
                $data = $res['data'];
                foreach ($data as $k => $v) {
                    if (in_array($k, ['bug_pub_at', 'hope_process_time'])) {
    ...
    public function actionAdd()
    {
        ...
        $params = isset($params['UserCloundComplaints']) ? $params['UserCloundComplaints'] : [];
        $model = new UserCloundComplaints();
        //是否通过验证，获取token
        $rs = $this->getAuth();
        if (!empty($rs) && $rs['code'] == 0) {
            //通过验证
            $url = $rs['prefix'] . self::COMPLAINTS_PUSH_CLOUD_API;
            $params['access_token'] = $rs['data']['access_token'];
            $params['uid'] = $rs['data']['uid'];
            $file = $_FILES;
            if (!empty($file['UserCloundComplaints']['name']['bug_attach'])) {
                unset($params['bug_attach']);
                $params['bug_attach'] = file_get_contents($file['UserCloundComplaints']['tmp_name']['bug_attach']);
            }

            $params['hope_process_time'] = strtotime($params['hope_process_time']);
            $response = Tool::postApi($url, http_build_query($params));
            ...
            if ($res['code'] == 0) {
                Yii::$app->getSession()->setFlash('success', Yii::t('app', 'complaints push success'));
            } else {
    ...
    public function actionIndexAll()
    {
        ...
        if (!empty($params['bug_pub_time'])) {
            $value = strtotime($params['bug_pub_time']);
            $query->andWhere("bug_pub_at > :pub", [":pub" => $value]);
        }
        if (!empty($params['bug_type'])) {
            $value = $params['bug_type'];
            $query->andWhere("bug_type = :type", [":type" => $value]);
        }
        if (!empty($params['bug_state']) || preg_match('/^0$/', $params['bug_state'])) {
            $value = $params['bug_state'];
            $query->andWhere("bug_state = :state", [":state" => $value]);
        }
        //分页
        //一页多少条
        $offset = isset($params['offset']) && $params['offset'] > 0 ? $params['offset'] : 10;
        ...
        if (isset($params['orderBy']) && array_key_exists($params['orderBy'], $model->searchField)) {
            ...
            $query->orderBy(['bug_pub_at' => SORT_DESC]);
        }
    ...
    public function actionEdit($id)
    {
        ...
        $model->hope_process_time = date('Y-m-d', $model->hope_process_time);
        $model->bug_pub_at = date('Y-m-d H:i:s', $model->bug_pub_at);

        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            $data = $_POST['UserCloundComplaints'];
            $attributes = [
                //需要更新的字段
                'bug_solution' => $data['bug_solution'],
                'bug_state' => $data['bug_state'],
                'bug_solution_time' => time()
            ];
            $count = UserCloundComplaints::updateAll($attributes, 'id=:id', array(':id' => $id));
            if ($count) {
