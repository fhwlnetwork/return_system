	...
	$.fn.slide = function(options) {
		...
		function changeNavList() {
			if (settings.nav) {
				...
				$(".navListBox li").removeClass("active").eq(index - 1).addClass("active");
			}
		};
		//自动切换
		function autoRun() {
		...
		$.fn.moveTo = function(index) {
			...
			var nowIndex = $(".navListBox li.active").attr('data-index');
			if (index < nowIndex) {
				moveWhere = 'left';
			} else {
				moveWhere = 'right';
		...
		function toLast(move) {
			switch (move) {
				case 'last':
					el.find('li:first').css('margin-left', (el.children().length * 100) + "%");
					el.changeSlide(settings, -el.children().length * 100);
					el.find('li:first').imgMoveSetting('right');
					setTimeout(function() {
						el.css('left', '0%');
						el.find('li:first').css('margin-left', '0');
						el.find('li:first').addClass('active').siblings().removeClass('active');
						changeNavList();
					}, settings.slideSpeed + 50);
					break;
				case 'first':
					el.find('li:last').css('margin-left', -(el.children().length * 100) + "%");
					el.changeSlide(settings, 100);
					el.find('li:last').imgMoveSetting('left');
					setTimeout(function() {
						el.css('left', -((el.children().length - 1) * 100) + "%");
						el.find('li:last').css('margin-left', '0');
						el.find('li:last').addClass('active').siblings().removeClass('active');
						changeNavList();
					}, settings.slideSpeed + 50);
					break;
			}
		...
		if (settings.nav) $(".navListBox li" + "[data-index=1]").addClass("active");
		$(".navListBox li").bind({
			click: function() {
				if (!isMove) {
					return;
